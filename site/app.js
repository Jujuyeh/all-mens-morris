...
function gameFileUrl() {
  ...
  return new URL("build/all-mens-morris-latest.hex", window.location.href).href;
}
...
function updateDeviceScale() {
  ...
  const available = Math.max(1, Math.min(530, document.documentElement.clientWidth - 24));
  const scale = Math.round(Math.min(1, available / 530) * 10000) / 10000;
  const height = Math.round(device.offsetHeight * scale);
  const width = Math.round(530 * scale);
...
function frameTargets(iframe = document.querySelector("#player")) {
  ...
  try {
    ...
    // Cross-origin previews cannot receive synthetic input.
  }
...
function eventTargets() {
  ...
  return [window, ...frameTargets(iframe)];
}
...
function focusPlayerCanvas() {
  ...
  try {
    iframe?.focus();
    iframe.contentDocument?.querySelector("canvas")?.focus();
  } catch (_) {
    // Same-origin previews can focus the canvas. Cross-origin previews cannot.
  }
}
...
function playerAudioApi() {
  const iframe = document.querySelector("#player");
  try {
    return iframe.contentWindow?.MorrisAudio;
  } catch (_) {
    return null;
  }
}
...
function unlockPlayerAudio() {
  ...
  if (api?.unlock) return api.unlock();

  postPlayerAudioMessage({ type: "morris:audio-unlock" });
...
function setPlayerMuted(muted) {
  ...
  const api = playerAudioApi();
  if (api?.setMuted) return api.setMuted(audioMuted);

  postPlayerAudioMessage({ type: "morris:audio-muted", muted: audioMuted });
...
function setButtonState(logicalKey, pressed) {
  ...
  if (control) control.classList.toggle("is-active", pressed);
}
...
function setupControls() {
  ...
  window.addEventListener("keydown", (event) => {
    ...
    unlockPlayerAudio();
    setButtonState(logicalKey, true);
    if (event.isTrusted) dispatchFrameKey("keydown", logicalKey);
  });
  ...
  window.addEventListener("keyup", (event) => {
    ...
    if (!logicalKey) return;
    event.preventDefault();
    if (!activeKeys.has(logicalKey)) setButtonState(logicalKey, false);
    if (event.isTrusted) dispatchFrameKey("keyup", logicalKey);
  });
}
